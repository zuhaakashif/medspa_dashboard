...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    with tab4:
        ...
        with col_sl:
            ...
            no_show_rate_input   = st.slider("Expected No-Show Rate",     0.05, 0.30, 0.12, 0.01, format="%.0f%%", help="Current observed rate is 13.7%")
            overbooking_rate     = st.slider("Overbooking Percentage",    0.00, 0.20, 0.05, 0.01, format="%.0f%%", help="Extra slots to schedule above capacity")
            demand_growth        = st.slider("Demand Growth Scenario",    0.00, 0.50, 0.10, 0.01, format="%.0f%%", help="Additional demand from marketing / waitlist")
            fill_rate_input      = st.slider("Gap-Fill Success Rate",     0.10, 1.00, 0.70, 0.05, format="%.0f%%", help="Fraction of idle gaps filled by last-minute bookings")
